...
async def handle_all_messages(message: Message):
    ...
    if user_id in user_deals:
        ...
        if deal_data.get("step") == "description":
            ...
            await message.answer("🛡 Создание сделки. Пример: 2 кепки дурова и ...\n\nВыберите валюту:" if lang == "ru" else "🛡 Creating deal. Example: 2 durov caps and ... \n\nChoose currency:", reply_markup=currency_keyboard_ru if lang == "ru" else currency_keyboard_en)
...
async def deal_type_callback(callback: CallbackQuery):
    ...
    lang = user_languages.get(callback.from_user.id, "ru")
    await safe_edit_message(callback, "🛡 Описание подарка. Пример: 2 кепки дурова и .." if lang == "ru" else "🛡 Gift description", back_keyboard_ru if lang == "ru" else back_keyboard_en)
